...
def _TS_train(teacher, student, dataset):
    distill_criterion = nn.CrossEntropyLoss()
    clf_criterion = nn.CrossEntropyLoss()
    teacher.eval()
    student.train()
    optimizer = optim.SGD(student.parameters(), lr=1e-4)
    for data, label in dataset:
        teacher_logits = teacher(data)
        student_logits = student(data)
        distill_loss = distill_criterion(teacher_logits, student_logits)
        clf_loss = clf_criterion(student_logits, label)
